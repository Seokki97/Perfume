...
public class RecommendService {
    ...
    public RecommendService(PerfumeRepository perfumeRepository, SurveyRepository surveyRepository, SurveyService surveyService) {
    ...
    public List<Survey> findSimilarPerfume(Long id) {
        ...
        List<Survey> addedList = surveyService.addList(firstFeatureList, surveyRepository.findByFirstAnswerOfSurvey("젠더리스"));
        List<Survey> comparedList = surveyService.compareTwoFilteredSurveyData(addedList, secondFeatureList);
        List<Survey> thirdData = surveyRepository.findByThirdAnswerOfSurveyContaining(findThirdFeatureSelectedPerfume(id));
        return surveyService.compareTwoFilteredSurveyData(comparedList,thirdData);
    }
